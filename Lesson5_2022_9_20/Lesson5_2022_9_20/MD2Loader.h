...
public:
    ...
    /** 返回3D模型结构 */
	md2_namespace::t3DModel& GetModel() { return m_3DModel; }

	/** 初始化 */
	void  Init(const char* strFileName, const char *strTexture);

	/** 播放模型动画 */
	void AnimateMD2Model();
    
	/** 模型结构 */
	md2_namespace::t3DModel                m_3DModel;                  
	
private:
	
	/** 载入MD2文件 */
	bool ImportMD2(md2_namespace::t3DModel *pModel,const  char *strFileName, const char *strTexture);
	...
	/** 保存动作信息 */
	void ParseAnimations(md2_namespace::t3DModel *pModel);

	/** 将MD2文件结构转化为模型结构 */
	void ConvertDataStructures(md2_namespace::t3DModel *pModel);

	/** 释放内存 */
	void CleanUp();

	/** 返回当前时间 */
	float ReturnCurrentTime(md2_namespace::t3DModel *pModel, int nextFrame);
