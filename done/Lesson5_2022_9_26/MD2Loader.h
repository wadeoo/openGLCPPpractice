...
public:
    ...
    /** 返回3D模型结构 */
	namespace_md2::t3DModel& GetModel() { return m_3DModel; }
	...
	/** 模型结构 */
	namespace_md2::t3DModel                m_3DModel;
	
private:
	
	/** 载入MD2文件 */
	bool ImportMD2(namespace_md2::t3DModel *pModel, char *strFileName, char *strTexture);
	...
	/** 保存动作信息 */
	void ParseAnimations(namespace_md2::t3DModel *pModel);

	/** 将MD2文件结构转化为模型结构 */
	void ConvertDataStructures(namespace_md2::t3DModel *pModel);

	/** 释放内存 */
	void CleanUp();

	/** 返回当前时间 */
	float ReturnCurrentTime(namespace_md2::t3DModel *pModel, int nextFrame);
